...
class GnuChessEngine:
	...
	def __init__(self):
		...
		try:
			if self.plat == "win32":
				self.proc = Popen(engine_args, executable=os.path.abspath(engine_path), stdin=PIPE, stdout=PIPE)
			else:
				self.proc = Popen(engine_args, executable=os.path.abspath(engine_path), stdin=PIPE, stdout=PIPE, close_fds=True)
